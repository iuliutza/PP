...
(define (enqueue x q)
  (if (> (add1 (queue-size-r q)) (queue-size-l q))
      (match q
        [(queue left right size-l size-r)
         (queue (stream-append (rotate left right '()) (list x))  '() (add1 (+ size-l size-r)) 0)])

      (match q
        [(queue left right size-l size-r)
         (queue left  (cons x (queue-right q)) size-l (add1 size-r))])))
...
(define (dequeue q)
  (if (> (add1 (queue-size-r q)) (queue-size-l q))
      (match q
        [(queue left right size-l size-r)
         (queue (stream-rest (rotate left right '())) '()  (- (+ size-l size-r) 1) 0)])
        
      (match q
        [(queue left right size-l size-r)
         (queue (stream-rest left) right  (- size-l 1) size-r)])))

; TODO
; Implementați o funcție care obține primul element dintr-o coadă nevidă
; (nu verificați că e nevidă, pe coada vidă este firesc să dea eroare).
; Veți întoarce elementul aflat la începutul cozii.
(define (top q) (stream-first (queue-left q)))
