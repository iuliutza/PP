...
(define (add-to-counter name n-items)
  (λ (C) 
          ...
          (match C
            [(counter index tt et queue)
            (counter index (+ n-items (counter-tt C)) (+ n-items (counter-et C)) (append (counter-queue C) (list (cons name n-items))))])
          (match C
            [(counter index tt et queue)
            (counter index (+ n-items (counter-tt C)) et (append (counter-queue C) (list (cons name n-items))))]))))
...
(define (min-tt counters)
  (if(null? (cdr counters))
     ...
     (min-thingy (cons (counter-index (car counters)) (counter-tt (car counters))) (min-tt (cdr counters))))) ; folosind funcția de mai sus

;(define (min-et counters)
;  (if(null? (cdr counters))
;     (cons (counter-index (car counters)) (counter-et (car counters)))
;     (min-thingy (cons (counter-index (car counters)) (counter-et (car counters))) (min-et (cdr counters))))) ; folosind funcția de mai sus

(define (min-et counters)
  (cond
    ;((null? (counter-queue (car counters))) (min-et (cdr counters)))
...
(define (remove-first-from-counter C)
  (cond
    ((null? (counter-queue C)) (match C
          [(counter index tt et queue)
          (counter index 0 0 '())]))
    ((null? (cdr (counter-queue C)))  (match C
          [(counter index tt et queue)
          (counter index 0 0 '())]))
    (else (match C
          [(counter index tt et queue)
          (counter index (- tt et) (cdr (cadr queue)) (remove (car queue) queue))]))))
...
(define (serve requests fast-counters slow-counters)
  ...
  (if (null? requests)
      ...
      (match (car requests)
        ...
        [(list 'ensure average)(let* ([new-s-counters (add-counters slow-counters average big-list)]) (serve (cdr requests) fast-counters new-s-counters))]                        
        
        [(list name n-items) (if (> n-items ITEMS)
                              (let* ([new-s-counters (update (add-to-counter name n-items) slow-counters (car (min-tt slow-counters)))]) (serve (cdr requests) fast-counters new-s-counters))
                              (let* ([new-f-counters (update (add-to-counter name n-items) fast-counters (car (min-tt big-list)))]
                                ...
                                (serve (cdr requests) new-f-counters new-s-counters)))]     
        ...
        [(list remove-first) (if (null? (filter (λ (C) (not (null? (counter-queue C)))) big-list))
                                 (serve (cdr requests) fast-counters slow-counters)
                                 (let* ([new-f-counters (update remove-first-from-counter fast-counters (car ( min-et (filter  (λ (C) (not (null? (counter-queue C)))) big-list))))]
                                        [new-s-counters (update remove-first-from-counter slow-counters (car ( min-et (filter  (λ (C) (not (null? (counter-queue C)))) big-list) )))]) 
                                   (serve (cdr requests) new-f-counters new-s-counters)))])))
