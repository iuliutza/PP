...
(define (copy-stack l r)
  (if (null? r)
      l
      (copy-stack (cons (car r) l) (cdr r))))

(define (dequeue q)
  (cond
    ((null? (queue-left q)) (match q
                               [(queue left right size-l size-r)
                                (queue (cdr (copy-stack left right)) '() (sub1 (+ size-l size-r)) 0)]))
    (else (match q
            [(queue left right size-l size-r)
             (queue (cdr left) right  (- size-l 1) size-r)]))))
...
(define (top q)
  (if (null? (queue-left q))
      (car (copy-stack (queue-left q) (queue-right q)))
      (car (queue-left q))))
